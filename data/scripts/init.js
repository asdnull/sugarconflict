...
function prepSceneData()
{
    if(main.data.H_RPGX)
    {
        for(let key in sceneData)
        {
            ...
            //Merge 'SCENE' metadata from data/meta.js, if any
            if(SCENE[key]){
                mergeMeta(curScene, key);
            }
            //Set if scene is saved as a favorite
            if(prefs.select.favourites.indexOf(key) > -1){
                curScene.favourite = true;
            }

            //Gets all 'EV' images in script
    ...
    //Merge metadata from data/meta.js
...
//Only used by the "Search" tab I think, uses the autocomplete.js file
...
//Uses the data/custom.js file to add/overwrite anything set in the data/meta.js file?
//But since this overwrites global variables, the custom.js file can then destroy the viewer by accidentally defining the wrong names
...
function initMeta()
{
    ...
    //Merges 'SCENE' metadata from data/meta.js, sets favorites and get EV images from script
    prepSceneData();
